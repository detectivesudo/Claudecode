...
HASH_SIGNATURES = [
    ("Argon2",       re.compile(r"^\$argon2(i|d|id)\$"), None),
    ("bcrypt",       re.compile(r"^\$2[ab]?\$\d{2}\$.{53}$"), None),
    ("scrypt",       re.compile(r"^\$scrypt\$"), None),
    ("PBKDF2-SHA256",re.compile(r"^\$pbkdf2-sha256\$"), None),
    ("PBKDF2-SHA512",re.compile(r"^\$pbkdf2-sha512\$"), None),
    ("Kerberos 5 TGT",re.compile(r"^\$krb5tgs\$"), None),
    ("Kerberos 5 AS-REP",re.compile(r"^\$krb5asrep\$"), None),
    ("WordPress (phpass)",re.compile(r"^\$P\$"), None),
    ("Cisco IOS (type 5)",re.compile(r"^\$1\$"), None),
    ("SHA-512 crypt",re.compile(r"^\$6\$"), None),
    ("SHA-256 crypt",re.compile(r"^\$5\$"), None),
    ("MD5 crypt",    re.compile(r"^\$1\$"), None),
    ("MySQL 4.1+",   re.compile(r"^\*[0-9A-F]{40}$"), 41),
    ("LM Hash",      re.compile(r"^[0-9A-F]{32}$"), 32),
    ("NTLM",         re.compile(r"^[0-9a-fA-F]{32}$"), 32),
    ("MD5",          re.compile(r"^[0-9a-f]{32}$"), 32),
    ("MD5 (upper)",  re.compile(r"^[0-9A-F]{32}$"), 32),
    ("MySQL 3.x",    re.compile(r"^[0-9a-f]{16}$"), 16),
    ("SHA-1",        re.compile(r"^[0-9a-f]{40}$"), 40),
    ("SHA-1 (upper)",re.compile(r"^[0-9A-F]{40}$"), 40),
    ("SHA-224",      re.compile(r"^[0-9a-f]{56}$"), 56),
    ("SHA-256",      re.compile(r"^[0-9a-f]{64}$"), 64),
    ("SHA-256 (upper)",re.compile(r"^[0-9A-F]{64}$"), 64),
    ("SHA3-256 / Keccak-256", re.compile(r"^[0-9a-f]{64}$"), 64),
    ("SHA-384",      re.compile(r"^[0-9a-f]{96}$"), 96),
    ("SHA-512",      re.compile(r"^[0-9a-f]{128}$"), 128),
    ("SHA-512 (upper)",re.compile(r"^[0-9A-F]{128}$"), 128),
    ("Base64-SHA1",  re.compile(r"^[A-Za-z0-9+/]{27}=$"), 28),
    ("CRC32",        re.compile(r"^[0-9a-f]{8}$"), 8),
    ("RIPEMD-160",   re.compile(r"^[0-9a-f]{40}$"), 40),
    ("Whirlpool",    re.compile(r"^[0-9a-f]{128}$"), 128),
    ("GOST R 34.11-94", re.compile(r"^[0-9a-f]{64}$"), 64),
]
